...
- `clone chat shell`
  `GET /chat`, `GET /api/clone-chat/status` ja `POST /api/clone-chat/respond` avaavat nyt IRC-henkisen testihuoneen kloonille; huone käyttää bounded-korpusvastausta paikallisesti ja voi renderöidä GPT-5.4:llä heti kun `OPENAI_API_KEY` on asetettu
...
Selainhuone löytyy nyt myös omasta URLista:
